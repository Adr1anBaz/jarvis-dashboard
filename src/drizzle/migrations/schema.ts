import { pgTable, foreignKey, unique, serial, integer, timestamp, text, boolean, varchar, check, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const estadoReporte = pgEnum("estado_reporte", ['pendiente', 'resuelto', 'descartado'])
export const grupoTipo = pgEnum("grupo_tipo", ['universidad', 'carrera', 'comunidad'])
export const motivoReporte = pgEnum("motivo_reporte", ['spam', 'acoso', 'contenido_inadecuado', 'falso', 'otro'])
export const rolUsuarioGrupo = pgEnum("rol_usuario_grupo", ['administrador', 'miembro'])
export const tipoContenidoOculto = pgEnum("tipo_contenido_oculto", ['aviso', 'publicacion_foro', 'respuesta_foro'])
export const tipoObjetivo = pgEnum("tipo_objetivo", ['respuesta_foro', 'publicacion_foro', 'aviso'])
export const tipoReaccion = pgEnum("tipo_reaccion", ['like', 'no_apropiado', 'no_me_interesa', 'mas_informacion', 'me_encanta', 'me_divierte'])


export const usuarioGrupo = pgTable("usuario_grupo", {
	...
	unique("usuario_grupo_usuario_id_grupo_id_key").on(table.usuarioId, table.grupoId),
]);
...
export const correoAsociado = pgTable("correo_asociado", {
	id: serial().primaryKey().notNull(),
...
export const foro = pgTable("foro", {
	id: serial().primaryKey().notNull(),
	grupoId: integer("grupo_id"),
	descripcion: text(),
}, (table) => [
...
export const respuestaForo = pgTable("respuesta_foro", {
	id: serial().primaryKey().notNull(),
...
export const publicacionForo = pgTable("publicacion_foro", {
	id: serial().primaryKey().notNull(),
...
export const reaccion = pgTable("reaccion", {
	id: serial().primaryKey().notNull(),
	...
	unique("reaccion_usuario_id_tipo_aviso_id_publicacion_foro_id_key").on(table.tipo, table.usuarioId, table.avisoId, table.publicacionForoId),
]);

export const reporte = pgTable("reporte", {
	id: serial().primaryKey().notNull(),
	...
	fechaReporte: timestamp("fecha_reporte", { mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
...
export const usuarioBloqueado = pgTable("usuario_bloqueado", {
	id: serial().primaryKey().notNull(),
	bloqueadorId: integer("bloqueador_id"),
	bloqueadoId: integer("bloqueado_id"),
	fechaBloqueo: timestamp("fecha_bloqueo", { mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
	razon: text(),
	...
	unique("usuario_bloqueado_bloqueador_id_bloqueado_id_key").on(table.bloqueadorId, table.bloqueadoId),
	check("usuario_bloqueado_check", sql`bloqueador_id <> bloqueado_id`),
]);

export const contenidoOculto = pgTable("contenido_oculto", {
	id: serial().primaryKey().notNull(),
	usuarioId: integer("usuario_id"),
	objectId: integer("object_id").notNull(),
	objectType: tipoContenidoOculto("object_type").notNull(),
	fechaOcultado: timestamp("fecha_ocultado", { mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
	foreignKey({
			columns: [table.usuarioId],
			foreignColumns: [usuario.id],
	...
	unique("contenido_oculto_usuario_id_object_type_object_id_key").on(table.usuarioId, table.objectId, table.objectType),
]);
